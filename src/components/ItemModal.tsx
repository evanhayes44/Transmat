...
export function ItemModal({
  ...
  const damageType = instance?.damageType
    ? damageTypes[instance.damageType]
    : null;
  ...
  const visibleSockets =
    sockets?.sockets.filter((s) => s.isVisible && s.plugHash) ?? [];

  const perkSockets = visibleSockets.filter((s) => {
    const cat =
      manifestData?.[String(s.plugHash)]?.plug?.plugCategoryIdentifier ?? "";
    return (
      cat !== "" &&
  ...
  const masterworkStat =
    masterworkPlugName && !masterworkPlugName.toLowerCase().includes("catalyst")
      ? masterworkPlugName.replace(/ masterwork$/i, "").trim()
      : null;

  const masterworkStats =
    masterworkPlugDef?.investmentStats
      ?.filter((s) => s.value > 0 && statNames[s.statTypeHash])
      .map((s) => ({ name: statNames[s.statTypeHash], value: s.value })) ?? [];

  const catalystSocket =
    isExotic && itemDef.itemType === 3
      ? visibleSockets.find((s) => {
          const cat =
            manifestData?.[String(s.plugHash)]?.plug?.plugCategoryIdentifier ??
            "";
          return cat.includes("masterwork");
  ...
  const ornamentIcon = (() => {
    ...
    for (const s of ornamentSockets) {
      const plugIcon =
        manifestData?.[String(s.plugHash)]?.displayProperties.icon;
  ...
  if (catalystSocket) {
    const objectives =
      plugObjectives?.objectivesPerPlug[String(catalystSocket.plugHash)];
    if (!objectives || objectives.length === 0) catalystStatus = "Not Unlocked";
    else if (objectives[0].complete) catalystStatus = "Complete";
    else
      catalystStatus = `In Progress — ${Math.floor((objectives[0].progress / objectives[0].completionValue) * 100)}%`;
  }

  function findItemLocation(
    instanceId: string,
  ): { type: "character"; charId: string } | { type: "vault" } {
    if (equippedItems) {
      for (const [charId, charItems] of Object.entries(equippedItems)) {
        if (charItems.some((i) => i.itemInstanceId === instanceId))
          return { type: "character", charId };
      }
    }
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
          ...
          <div className={styles.headerLeft}>
            {displayIcon && (
              <img
                ...
                src={`https://www.bungie.net${displayIcon}`}
                alt={name}
              />
            )}
            <div className={styles.title}>
              <span
                className={`${styles.name}${isMasterwork ? ` ${styles.nameMasterwork}` : ""}`}
              >
                {name}
              </span>
              {subTypeName && (
                <span className={styles.type}>{subTypeName}</span>
              )}
              <div className={styles.meta}>
                {power && <span className={styles.power}>◆ {power}</span>}
                {damageType && (
                  <span
                    className={`${styles.element} element-${damageType.toLowerCase()}`}
                  >
                    {damageType}
                  </span>
                )}
        ...
        <div className={styles.body}>
          <div className={styles.left}>
                  ...
                  <div key={hash} className="item-tooltip-stat">
                    ...
                    {name !== "RPM" && (
                      <div className="item-tooltip-stat-bar">
                        <div
                          className="item-tooltip-stat-fill"
            ...
            {perkSockets.length > 0 && (
              <div className="item-tooltip-perks">
                {perkSockets.map((socket, index) => {
                  ...
                  return (
                    <div
                      key={`${socket.plugHash}-${index}`}
                      className="item-tooltip-perk-icon-wrap"
                    >
                      {plugDef.displayProperties.icon && (
                        <img
                          src={`https://www.bungie.net${plugDef.displayProperties.icon}`}
                          alt={plugDef.displayProperties.name}
                        />
                      )}
                      <div className="item-tooltip-perk-sub">
                        <span className="item-tooltip-perk-name">
                          {plugDef.displayProperties.name}
                        </span>
                        {plugDef.displayProperties.description && (
                          <span className="item-tooltip-perk-desc">
                            {plugDef.displayProperties.description}
                          </span>
                        )}
                      </div>
            ...
            {(catalystSocket || ornamentSockets.length > 0) && (
              <div className="item-tooltip-perks item-tooltip-perks-special">
                {catalystSocket &&
                  (() => {
                    const plugDef =
                      manifestData?.[String(catalystSocket.plugHash)];
                    if (!plugDef) return null;
                    return (
                      <div className="item-tooltip-perk-icon-wrap">
                        ...
                        <div className="item-tooltip-perk-sub">
                          <span className="item-tooltip-catalyst-label">
                            Catalyst —{" "}
                            <span
                              className={
                                catalystStatus === "Complete" ? "complete" : ""
                              }
                            >
                              {catalystStatus}
                            </span>
            ...
            {isMasterwork && itemDef.itemType === 3 && masterworkStat && (
              <div className={styles.masterworkBadge}>
                <div className={styles.masterworkHeader}>
                  ...
                  <span>
                    ◆ {masterworkStat ? ` ${masterworkStat.toUpperCase()}` : ""}
                  </span>
                </div>
            ...
            {description && (
              <p className="item-tooltip-flavor">{description}</p>
            )}
          </div>
                    ...
                    return (
                      <div className={styles.slotGrid}>
                        {Array.from({ length: 9 }, (_, i) => {
                          ...
                          const slotDef = slotItem
                            ? manifestData?.[String(slotItem.itemHash)]
                            : undefined;
                          if (!slotDef?.displayProperties.icon)
                            return <div key={i} className={styles.slot} />;
                          const slotInstance =
                            itemInstances?.[slotItem.itemInstanceId];
