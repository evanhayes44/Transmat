...
import type {
  DestinyItem,
  DestinyProfileInventoryResponse,
...
export function useInventory(refreshKey?: number) {
  ...
  useEffect(() => {
    ...
    async function loadInventory() {
      ...
      try {
        const response = await bungieGet<DestinyProfileInventoryResponse>(
          `/Destiny2/${membershipType}/Profile/${destinyMembershipId}/?components=${components}`,
        );

        const rawData = response.Response.characterEquipment.data as Record<
          string,
          { items: DestinyItem[] }
        >;
        const items = Object.fromEntries(
          Object.entries(rawData).map(([characterId, value]) => [
            characterId,
            value.items,
          ]),
        );

        const vaultData = response.Response.profileInventory.data
          .items as DestinyItem[];

        const rawInventory = response.Response.characterInventories
          .data as Record<string, { items: DestinyItem[] }>;
        const characterInventory = Object.fromEntries(
          Object.entries(rawInventory).map(([characterId, value]) => [
            characterId,
            value.items,
          ]),
        );
        ...
        if (!isRefresh) {
          ...
          const itemPlugObjectives =
            response.Response.itemComponents.plugObjectives.data;
