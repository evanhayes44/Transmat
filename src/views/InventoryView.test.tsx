...
import type { DestinyItem, DestinyItemDefinition, DestinyCharacter } from '../types/bungie.types'
...
const mockGjallarhornDef: DestinyItemDefinition = {
    displayProperties: { name: 'Gjallarhorn', icon: '/img/gjallarhorn.png', description: '' },
    itemType: 3,
    itemSubType: 10,
...
describe('InventoryView', () => {
    ...
    describe('layout', () => {
        ...
        it('renders the Exotic filter button', () => {
            renderView()
            expect(screen.getByRole('button', { name: 'Exotic' })).toBeInTheDocument()
        })

        it('renders the Masterwork filter button', () => {
            renderView()
            expect(screen.getByRole('button', { name: '◆ Masterwork' })).toBeInTheDocument()
        })
    ...
    describe('sort dropdown', () => {
        ...
        it('updates to Power ↑ when selected', () => {
            renderView()
            fireEvent.change(screen.getByDisplayValue('Power ↓'), { target: { value: 'power-asc' } })
            expect(screen.getByDisplayValue('Power ↑')).toBeInTheDocument()
        })

        it('updates to Name A–Z when selected', () => {
            renderView()
            fireEvent.change(screen.getByDisplayValue('Power ↓'), { target: { value: 'name' } })
    ...
    describe('Exotic filter button', () => {
        it('starts without the active class', () => {
            renderView()
            expect(screen.getByRole('button', { name: 'Exotic' }).className).not.toContain('active')
        })
    ...
    describe('loading state', () => {
        it('renders 3 skeleton character panels when isLoading is true', () => {
            mockUseInventoryStore.mockReturnValue({ ...defaultInventoryState, isLoading: true })
            renderView()
    ...
    describe('character panels', () => {
        function setupCharacter(character: DestinyCharacter) {
            ...
            mockUseCharacterStore.mockReturnValue({ characters: { [character.characterId]: character } })
        }

        it('renders the character class name (Hunter)', () => {
            setupCharacter({ characterId: 'char-1', classType: 1, light: 1810, emblemBackgroundPath: '/emblem.png', raceType: 0, genderType: 0 })
            renderView()
            expect(screen.getByText('Hunter')).toBeInTheDocument()
        })

        it('renders the character class name (Warlock)', () => {
            setupCharacter({ characterId: 'char-1', classType: 2, light: 1820, emblemBackgroundPath: '/emblem.png', raceType: 0, genderType: 0 })
            renderView()
            expect(screen.getByText('Warlock')).toBeInTheDocument()
        })

        it('renders the character power level', () => {
            setupCharacter({ characterId: 'char-1', classType: 0, light: 1825, emblemBackgroundPath: '/emblem.png', raceType: 0, genderType: 0 })
            renderView()
    ...
    describe('item modal', () => {
        function setupVaultItem() {
            ...
            mockUseManifestStore.mockReturnValue({ data: mockManifest, titleData: null })
        }
