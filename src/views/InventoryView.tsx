...
export function InventoryView() {
  ...
  const { characters, error: charError } = useCharacterStore();
  ...
  const [sortBy, setSortBy] = useState<"power-desc" | "power-asc" | "name">(
    "power-desc",
  );
  ...
  const loadError = charError || invError;
  ...
  const vaultMisc =
    vaultItems?.filter(
      (item) =>
        manifestData?.[String(item.itemHash)]?.itemType !== 2 &&
        manifestData?.[String(item.itemHash)]?.itemType !== 3,
    ) ?? [];
    ...
    <div className={styles.page}>
      ...
      <div className={styles.characters}>
        {isLoading ? (
          ...
          items &&
          Object.entries(items).map(([characterId, itemArray]) => {
            const character = characters?.[characterId];
            const className = character
              ? classNames[character.classType]
              : characterId;

            const genderKey = character?.genderType === 1 ? "Female" : "Male";
            const characterTitle =
              titleData?.[String(character?.titleRecordHash)]?.titleInfo
                ?.titlesByGender?.[genderKey];

            const charInventory = characterInventory?.[characterId] ?? [];

            const charKinetic = charInventory.filter(
              (item) => item.bucketHash === bucketHashes.kinetic,
            );
            const charEnergy = charInventory.filter(
              (item) => item.bucketHash === bucketHashes.energy,
            );
            const charPower = charInventory.filter(
              (item) => item.bucketHash === bucketHashes.power,
            );
            const charHelmet = charInventory.filter(
              (item) => item.bucketHash === bucketHashes.helmet,
            );
            const charGauntlets = charInventory.filter(
              (item) => item.bucketHash === bucketHashes.gauntlets,
            );
            const charChest = charInventory.filter(
              (item) => item.bucketHash === bucketHashes.chest,
            );
            const charLegs = charInventory.filter(
              (item) => item.bucketHash === bucketHashes.legs,
            );
            const charClassItem = charInventory.filter(
              (item) => item.bucketHash === bucketHashes.classItem,
            );
              ...
              <div key={characterId} className={styles.characterPanel}>
                ...
                >
                  <div className={styles.characterHeaderOverlay}>
                    ...
                    <span className={styles.characterPower}>
                      ◆ {character?.light}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="inv-slot">
                    {(() => {
                      const item = itemArray.find(
                        (i) => i.bucketHash === bucketHashes.kinetic,
                      );
                      ...
                      const itemName =
                        ...
                        String(item.itemHash);
                      return (
                        <div
                          key={item.itemInstanceId}
                          className={`inv-item inv-item-equipped${(item.state & 4) !== 0 ? " inv-item-masterwork" : ""}`}
                          onClick={() => setSelectedItem(item)}
                        >
                          <img src={iconUrl} alt={itemName} />
                          {(() => {
                            const gearTier =
                              itemInstances?.[item.itemInstanceId]?.gearTier;
                          ...
                          {(() => {
                            const power =
                              itemInstances?.[item.itemInstanceId]?.primaryStat
                                ?.value;
                            if (!power) return null;
                            return (
                              <span className="inv-item-power">{power}</span>
                            );
                          })()}
                    ...
                    <div className="inv-slot-grid">
                      {Array.from({ length: 9 }, (_, i) => {
                        ...
                        if (!item)
                          return <div key={i} className="inv-item-empty" />;
                        const itemDef = manifestData?.[String(item.itemHash)];
                        const iconUrl = `https://www.bungie.net${itemDef?.displayProperties?.icon}`;
                        const itemName =
                          itemDef?.displayProperties?.name ??
                        ...
                        return (
                          <div
                            key={item.itemInstanceId}
                            className={`inv-item${(item.state & 4) !== 0 ? " inv-item-masterwork" : ""}`}
                            onClick={() => setSelectedItem(item)}
                          >
                            <img src={iconUrl} alt={itemName} />
                            {(() => {
                ...
                {(
                  [
                    {
                      label: "Energy",
                      bucket: bucketHashes.energy,
                      inv: charEnergy,
                    },
                    {
                      label: "Power",
                      bucket: bucketHashes.power,
                      inv: charPower,
                    },
                    {
                      label: "Helmet",
                      bucket: bucketHashes.helmet,
                      inv: charHelmet,
                    },
                    {
                      label: "Gauntlets",
                      bucket: bucketHashes.gauntlets,
                      inv: charGauntlets,
                    },
                    {
                      label: "Chest",
                      bucket: bucketHashes.chest,
                      inv: charChest,
                    },
                    { label: "Legs", bucket: bucketHashes.legs, inv: charLegs },
                    {
                      label: "Class Item",
                      bucket: bucketHashes.classItem,
                      inv: charClassItem,
                    },
                  ] as const
                ).map(({ label, bucket, inv }) => (
                  <div key={label} className="inv-section">
                    <p className="inv-section-label">{label}</p>
                    <div className="inv-slot">
                      {(() => {
                        const item = itemArray.find(
                          (i) => i.bucketHash === bucket,
                        );
                        if (!item) return <div className="inv-item-empty" />;
                        const itemDef = manifestData?.[String(item.itemHash)];
                        const iconUrl = `https://www.bungie.net${itemDef?.displayProperties?.icon}`;
      ...
      <div className={styles.vault}>
        ...
        <div className={styles.vaultControls}>
          ...
          <select
            ...
            value={filterElement ?? ""}
            onChange={(e) =>
              setFilterElement(e.target.value ? Number(e.target.value) : null)
            }
          >
          ...
          {Object.entries(weaponSubTypes).map(([key, name]) => (
            <button
              ...
              onClick={(e) => {
                (e.currentTarget as HTMLButtonElement).blur();
                setFilterWeaponType(
                  filterWeaponType === Number(key) ? null : Number(key),
                );
              }}
            >
              {name}
            ...
            <div className="inv-item-list">
              {isLoading
                ...
                : inv.map((item) => {
                    ...
                    const iconUrl = `https://www.bungie.net${itemDef?.displayProperties?.icon}`;
                    const itemName =
                      itemDef?.displayProperties?.name ?? String(item.itemHash);
                    return (
                      <div
                        key={item.itemInstanceId}
                        className={`inv-item${(item.state & 4) !== 0 ? " inv-item-masterwork" : ""}`}
        ...
        <div className="inv-item-list">
          {vaultMisc.map((item) => {
            ...
            return (
              <div
                key={item.itemInstanceId || String(item.itemHash)}
                className="inv-item"
                onClick={() => setSelectedItem(item)}
              >
                <img src={iconUrl} alt={itemName} />
      ...
      {selectedItem && (
        <ItemModal
          ...
          plugObjectives={
            itemPlugObjectives?.[selectedItem.itemInstanceId] ?? undefined
          }
